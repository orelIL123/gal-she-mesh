...
function HomeScreen({ onNavigate, isGuestMode = false }: HomeScreenProps) {
  ...
  // Get original images array - prioritize Firebase Storage images
  ...
  const fetchImages = async () => {
      try {
        ...
        // Load atmosphere and about us images from settings document
        let atmosphereImage = '';
        let aboutUsImage = '';
        ...
        if (settingsDocSnap.exists()) {
          ...
          console.log('📁 No settings/images document found - will try to load from Storage');
          
          // If no settings document, try to load from Firebase Storage
          try {
            const { getAllStorageImages } = await import('../../services/firebase');
            const storageImages = await getAllStorageImages();
            
            // Use first image from each category
            if (storageImages.aboutus.length > 0) {
              aboutUsImage = storageImages.aboutus[0];
              console.log('✅ Loaded aboutUs image from Storage');
            }
            if (storageImages.backgrounds.length > 0) {
              atmosphereImage = storageImages.backgrounds[0];
              console.log('✅ Loaded atmosphere image from Storage');
            }
          } catch (storageError) {
            console.log('❌ Error loading images from Storage:', storageError);
          }
        }
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.backgroundWrapper}>
        <ImageBackground
          source={settingsImages.atmosphere ? { uri: settingsImages.atmosphere } : { uri: 'https://via.placeholder.com/400x200/cccccc/666666?text=Loading+Atmosphere' }}
        ...
        <View style={styles.contentWrapper}>
          ...
          <Animated.View style={[styles.aboutSection, { opacity: cardsFade }]}>
            ...
            <View style={styles.aboutCard}>
              <Image
                source={settingsImages.aboutUs ? { uri: settingsImages.aboutUs } : { uri: 'https://via.placeholder.com/300x200/cccccc/666666?text=Loading+About+Us' }}
