...
import { Barber, getBarbers, getAllStorageImages } from '../services/firebase';
  ...
  const loadHeroImage = async () => {
    try {
      ...
      if (storageImages.backgrounds.length > 0) {
        setHeroImage(storageImages.backgrounds[0]);
      } else if (storageImages.atmosphere.length > 0) {
        setHeroImage(storageImages.atmosphere[0]);
      }
    } catch (error) {
      console.log('Error loading hero image:', error);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.heroSection}>
        <Image
          source={heroImage ? { uri: heroImage } : { uri: 'https://via.placeholder.com/400x200/cccccc/666666?text=Loading+Hero+Image' }}
