...
const AdminTeamScreen: React.FC<AdminTeamScreenProps> = ({ onNavigate, onBack }) => {
  ...
  const loadBarbers = async () => {
    try {
      ...
      // Auto-fix Naor Amar's image if needed
      const naoramar = barbersData.find(b => b.name === 'Naor Amar');
      if (naoramar && imagesData.length > 0 && !naoramar.image) {
        const naoramarImage = imagesData.find(img => img.includes('naoramar'));
        if (naoramarImage) {
          console.log('Auto-fixing Naor Amar image:', naoramarImage);
          try {
            await updateBarberProfile(naoramar.id, { image: naoramarImage });
            // Reload barbers to reflect changes
            const updatedBarbers = await getBarbers();
            setBarbers(updatedBarbers);
          } catch (error) {
            console.error('Error updating Naor Amar image:', error);
          }
        }
      }
